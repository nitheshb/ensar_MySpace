...
export default function EmpTableRow({
  row,
  selected,
}: Props) {
    ...
    <>
      <TableRow hover selected={selected}>
        ...
        <TableCell sx={{ display: 'flex', alignItems: 'center' , width: '300px'}}>
          <Avatar alt={row?.first_name} src={avatarUrl} sx={{ mr: 2 }} />

          <ListItemText
            primary={`${row?.first_name} ${row?.lastname}`}
            secondary={row?.company_email}
        ...
        <TableCell sx={{ whiteSpace: 'nowrap' }}> <ListItemText
            primary={row?.exit_date}
            secondary={row?.join_date}
            primaryTypographyProps={{ typography: 'body2' }}
            secondaryTypographyProps={{ component: 'span', color: 'text.disabled' }}
          /></TableCell>
        <TableCell sx={{ whiteSpace: 'nowrap' }}> <ListItemText
